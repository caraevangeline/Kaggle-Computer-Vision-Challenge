...
run inference via predict.py — a 3LC test table is not part of the competition
workflow.

Idempotent: if tables already exist, skips creation and prints the latest URLs.

Prerequisites (see README): 3lc installed, ``3lc login`` completed, and data
placed under ``data/``. All names and paths come from ``config.yaml``.
"""
...
def main() -> int:
    """Register (or verify) 3LC tables for the train and val splits."""
    os.chdir(WORK_DIR)
